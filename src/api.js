	...
	Object.keys ( localCircusConfig.tables ).forEach ( tableName => {
		...
		if ( fieldsConfig ) {
			Object.keys ( fieldsConfig ).forEach ( fieldName => {
				...
				if ( vuexTree.state.database.tables[tableName] && vuexTree.state.database.tables[tableName].fields_config && vuexTree.state.database.tables[tableName].fields_config[fieldName] )
